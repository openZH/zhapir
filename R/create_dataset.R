...
create_dataset <- function(
  ...
  # 2. Convert any name-based inputs to numeric IDs before object construction
  keyword_ids_num     <- if (is.character(keyword_ids))     convert_keywords_to_id(keyword_ids)       else keyword_ids
  zh_web_catalogs_num <- if (is.character(zh_web_catalogs)) convert_zh_web_catalog_to_id(zh_web_catalogs) else zh_web_catalogs
  theme_ids_num       <- if (is.character(theme_ids))       convert_themes_to_id(theme_ids)           else theme_ids
  periodicity_id_num  <- if (is.character(periodicity_id))  convert_periodicities_to_id(periodicity_id) else periodicity_id
  see_also_ids_num    <- if (is.character(see_also_ids))    convert_datasets_to_id(see_also_ids)       else see_also_ids
  ...
  ds <- Dataset(
    ...
    keyword_ids = keyword_ids_num,
    zh_web_catalogs = zh_web_catalogs_num,
    relation_ids = relation_ids,
    theme_ids = theme_ids_num,
    periodicity_id = periodicity_id_num,
    see_also_ids = see_also_ids_num
  )
